 ...
 * Resp: { used, limit, remaining, unlimited }
 */
...
export async function GET() {
  ...
  try {
    ...
    const { used, limit } = await checkUploadQuota(userId, session.user.role);
    ...
    return NextResponse.json({ used, limit, remaining, unlimited });
  } catch (err) {
