...
export default function AdminMusicasPage() {
  ...
  const authenticateWith = async (candidate: string, silent = false) => {
    if (!candidate) { if (!silent) setAuthError("Digite a senha"); return; }
    ...
    if (res.status !== 401) {
      setAdminPassword(candidate); // persiste p/ navegar entre módulos sem relogar
  ...
  const authenticate = () => authenticateWith(password);

  // Reaproveita a senha já salva pelos outros módulos (sessionStorage).
  useEffect(() => {
    const saved = getAdminPassword();
    if (saved) authenticateWith(saved, true);
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)" }}>
      ...
      <header style={{ background: "var(--surface)", borderBottom: "1px solid var(--border)" }}>
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center gap-4">
          <Link href="/admin" style={{ color: "var(--muted)", fontSize: 22 }} className="hover:opacity-70">←</Link>
          <h1 style={{ color: "var(--text)", fontWeight: 800, fontSize: 18, flex: 1 }}>Admin · Músicas</h1>
          <button
            onClick={newSong}
        ...
        <div className="max-w-4xl mx-auto px-4 pb-3 flex items-center gap-2 flex-wrap">
          {ADMIN_NAV.map((n) => {
            const active = n.href === "/admin/musicas";
            return (
