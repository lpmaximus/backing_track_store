...
export default function PerfilContent() {
  ...
  async function toggleShare(song: MySong) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(d.error ?? "Não foi possível alterar o compartilhamento.");
      }
    } finally {
      setBusyId(null);
    }
  ...
  async function remove(song: MySong) {
    if (song.shared) {
      alert("Descompartilhe a música antes de apagá-la.");
      return;
    }
    if (!confirm(`Apagar "${song.title}"? Essa ação não pode ser desfeita e remove os stems e a cifra.`)) return;
    setBusyId(song.id);
    try {
    ...
    <div style={{ flex: 1, maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px", width: "100%" }}>
      {status === "loading" ? null : !session?.user ? (
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 28, textAlign: "center" }}>
          <p style={{ color: "var(--muted)", fontSize: 14, margin: "0 0 14px" }}>Entre na sua conta para ver seu perfil.</p>
        ...
        <>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, gap: 16 }}>
            <div>
              <h2 style={{ fontWeight: 800, fontSize: 17, margin: "0 0 2px", color: "var(--text)" }}>Minhas músicas</h2>
              <p style={{ color: "var(--muted)", fontSize: 13, margin: 0 }}>Faixas que você enviou e converteu em backing track</p>
            </div>
          ...
          {loading ? (
            ...
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 40, textAlign: "center" }}>
              <p style={{ color: "var(--muted)", fontSize: 14, margin: "0 0 14px", fontStyle: "italic" }}>
                Você ainda não tem nenhuma faixa convertida.
              </p>
              <Link href="/upload" className="btn-primary" style={{ padding: "9px 22px", fontSize: 13, display: "inline-block" }}>Enviar a primeira</Link>
            </div>
          ) : (
  ...
  return (
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "16px 20px" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
        <Link href={`/song/${song.slug}`} style={{ minWidth: 0, color: "inherit", flex: 1 }}>
        ...
        {!editing && (
          <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            ...
            <button onClick={onEdit} disabled={busy} style={iconBtn}>✎ Editar</button>
            <button
              onClick={onRemove}
              disabled={busy || song.shared}
              title={song.shared ? "Descompartilhe antes de apagar" : "Apagar"}
              style={{
                ...iconBtn,
