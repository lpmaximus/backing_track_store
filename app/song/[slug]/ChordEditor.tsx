...
export default function ChordEditor({
  ...
  async function save() {
    ...
    const cleaned = rows
      ...
      .filter((r) => r.chords);
    if (cleaned.length === 0) {
      setError("Adicione ao menos uma linha com acordes.");
      return;
    }
    setSaving(true);
    setError("");
  ...
  return (
    <div style={{ padding: "16px 20px" }}>
      <p style={{ fontWeight: 700, fontSize: 12, letterSpacing: "0.08em", color: "var(--muted)", margin: "0 0 12px" }}>
        SUGERIR CORREÇÃO DA CIFRA
      </p>
      <p style={{ color: "var(--muted2)", fontSize: 12, margin: "0 0 14px" }}>
        Cada linha é um trecho: rótulo (opcional), tempo em segundos e os acordes separados por espaço.
      </p>

      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {rows.map((r, i) => (
          <div key={i} style={{ display: "flex", gap: 8, alignItems: "center" }}>
            <input style={{ ...inputStyle, width: 120 }} placeholder="Verso/Refrão" value={r.section} onChange={(e) => update(i, { section: e.target.value })} />
            ...
            <button onClick={() => removeRow(i)} title="Remover" style={{ background: "none", border: "none", color: "var(--muted2)", cursor: "pointer", fontSize: 18, padding: "0 4px" }}>×</button>
          </div>
        ))}
      </div>
