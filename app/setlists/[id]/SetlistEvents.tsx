    ...
    <div style={{ marginTop: 28, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 20 }}>
        ...
        <div style={{ border: "1px solid var(--border2)", borderRadius: 10, padding: 14, marginBottom: 16, background: "var(--surface2)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            ...
            <input
              ...
              placeholder="Título (ex: Ensaio geral)"
              maxLength={200}
              style={inputStyle}
            />
          ...
          <textarea
            ...
            rows={2}
            placeholder="Objetivo do ensaio (ex: fechar a virada de Sultans e a ponte de Hotel California)"
      ...
      {loading ? (
        ...
        <p style={{ color: "var(--muted2)", fontSize: 13, fontStyle: "italic", margin: 0 }}>
          {canManage
            ? "Nenhum ensaio marcado. Crie um para distribuir o que cada integrante vai treinar."
            : "Nenhum ensaio marcado pelo líder ainda."}
        </p>
      ) : (
...
function EventCard({
  ...
  return (
    <Link
      href={`/setlists/${setlistId}/ensaios/${ev.id}`}
      style={{
        display: "block",
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <span style={{ color: "var(--muted)", fontSize: 12 }}>{formatWhen(ev.startsAt)}</span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 6, flexWrap: "wrap" }}>
          {ev.assignedCount > 0 && (
            <span style={{ fontSize: 12, color: "var(--muted)" }}>
              🟢 {ev.readyCount} de {ev.assignedCount} prontos
            </span>
          )}
          {ev.myAssignments > 0 && (
            <span style={{ fontSize: 12, color: "var(--accent)", fontWeight: 700 }}>
              {ev.myAssignments} {ev.myAssignments === 1 ? "música sua" : "músicas suas"}
            </span>
          )}
          {pending && (
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--danger)" }}>confirme presença</span>
          )}
        </div>
      )}
    </Link>
