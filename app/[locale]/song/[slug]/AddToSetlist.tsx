      ...
      {open && (
        <>
          ...
          <div style={{
            ...
            {loading ? (
              <p style={{ color: "var(--muted)", fontSize: 13, textAlign: "center", padding: "12px 0", margin: 0 }}>{tc("loading")}</p>
            ) : !items || items.length === 0 ? (
              <div style={{ textAlign: "center", padding: "10px 6px" }}>
                <p style={{ color: "var(--muted)", fontSize: 13, margin: "0 0 8px" }}>{t("noSetlists")}</p>
                <Link href="/setlists" style={{ color: "var(--accent)", fontWeight: 700, fontSize: 13 }}>{t("createFirstSetlist")}</Link>
              </div>
            ) : (
              <>
                {items.map(s => {
                  ...
                  return (
                    <button key={s.id} onClick={() => !added && addTo(s.id)} disabled={adding === s.id || added}
                      ...
                      <span style={{ color: added ? "var(--accent)" : "var(--muted2)", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                        {added ? t("added") : adding === s.id ? "..." : tc("add")}
                      </span>
                    </button>
                  );
                })}
