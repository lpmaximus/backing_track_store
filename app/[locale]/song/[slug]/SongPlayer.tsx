      ...
      <div style={{ flex: 1, maxWidth: 1200, margin: "0 auto", padding: "24px 24px 40px", width: "100%", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        {soloInstrument && isPro && stems.some(s => s.instrument === soloInstrument) && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "rgba(255,154,0,0.08)", border: "1px solid rgba(255,154,0,0.25)", borderRadius: 8, fontSize: 13, color: "var(--muted)" }}>
            ...
            <span><strong>{t("guideTitle")}</strong>{t("guideText")}</span>
          </div>
        )}
        ...
        {setlistMix && isPro && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 14px", background: "rgba(255,154,0,0.08)", border: "1px solid rgba(255,154,0,0.25)", borderRadius: 8, fontSize: 13, color: "var(--muted)", flexWrap: "wrap" }}>
            ...
            <span>
              {setlistName
                ? t.rich("mixApplied", { name: setlistName, b: (c) => <strong>{c}</strong> })
                : t("mixAppliedNoName")}
              {setlistMix.some(m => m.source === "auto") && t("mixAutoMuted")}
              {setlistTranspose !== 0 && t("mixKey", { value: setlistTranspose > 0 ? `+${setlistTranspose}` : String(setlistTranspose) })}
              {setlistSpeed !== 1 && t("mixSpeed", { value: setlistSpeed.toFixed(2) })}
              .
            </span>
            <Link href={{ pathname: "/song/[slug]", params: { slug: song.slug } }} style={{ color: "var(--accent)", fontWeight: 600, marginLeft: "auto" }}>
              {t("useOriginalMix")}
            </Link>
          </div>
        )}
        ...
        <div className="player-content" style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={fullscreen
              ...
              <div style={{ borderBottom: "1px solid var(--border)", padding: "10px 16px", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                <span style={{ background: "var(--accent)", color: "#000", border: "1px solid var(--border2)", borderRadius: 6, padding: "3px 12px", fontSize: 11, fontWeight: 700, letterSpacing: "0.06em" }}>{t("chordsTab")}</span>
                {(isDraft && hasChords) || (lyricsIsDraft && hasLyrics) ? (
                  <span style={{ background: "rgba(245,158,11,0.15)", color: "#b45309", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 4 }}>{t("badgeAuto")}</span>
                ) : hasChords ? (
                  <span style={{ background: "rgba(255,154,0,0.15)", color: "var(--accent)", fontSize: 11, fontWeight: 700, padding: "2px 8px", borderRadius: 4 }}>{t("badgeSynced")}</span>
                ) : null}
                {isPro && !editing && (
                  <button onClick={() => setEditing(true)} style={{ background: "var(--surface2)", border: "1px solid var(--border2)", borderRadius: 6, padding: "3px 10px", fontSize: 12, cursor: "pointer", color: "var(--text)", fontWeight: 600 }}>{t("fixChart")}</button>
                )}
                {isAuth && hasChords && (
                ...
                <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 6 }}>
                  ...
                  <button onClick={() => setFullscreen(v => !v)} title={fullscreen ? t("exitFullscreenTitle") : t("fullscreenTitle")} style={{ background: "var(--surface2)", border: "1px solid var(--border2)", borderRadius: 6, padding: "3px 10px", fontSize: 13, cursor: "pointer", color: "var(--text)", fontWeight: 600 }}>{fullscreen ? "✕" : "⛶"}</button>
                </div>
              </div>
              {/* Content */}
              <div
                  ...
                  : (hasChords || hasLyrics)
                    ...
                    : song.cifraText
                      ...
                      : (generating || generatingLyrics)
                        ? <p style={{ color: "var(--muted)", fontStyle: "italic", textAlign: "center", marginTop: 40 }}>{t("generating")}</p>
                        : <p style={{ color: "var(--muted)", fontStyle: "italic", textAlign: "center", marginTop: 40 }}>{t("noChart")}</p>
                }
              </div>
              ...
              {fullscreen && (
                <div style={{ borderTop: "1px solid var(--border)", padding: "10px 16px", display: "flex", alignItems: "center", gap: 14, background: "var(--surface)", flexWrap: "wrap" }}>
                  <button onClick={handleAutoFollowToggle} title={t("autoFollowHint")} style={{ background: autoFollow ? "var(--accent)" : "transparent", color: autoFollow ? "#000" : "var(--text)", border: `1.5px solid ${autoFollow ? "var(--accent)" : "var(--border2)"}`, borderRadius: 8, padding: "8px 18px", fontWeight: 700, fontSize: 13, cursor: "pointer" }}>
                    {t("autoFollow")}
                  </button>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1, minWidth: 160, maxWidth: 320, opacity: autoFollow ? 0.4 : 1 }}>
                    ...
                    <span style={{ color: "var(--muted)", fontSize: 12 }}>{t("fast")}</span>
                    <span style={{ color: "var(--text)", fontSize: 12, fontWeight: 700, minWidth: 38 }}>{scrollSpd}x</span>
                  </div>
                  <button onClick={() => setFullscreen(false)} style={{ marginLeft: "auto", background: "var(--surface2)", border: "1px solid var(--border2)", borderRadius: 8, padding: "8px 16px", fontSize: 13, cursor: "pointer", color: "var(--text)", fontWeight: 600 }}>{t("exitFullscreen")}</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="player-sidebar" style={{ width: 210, flexShrink: 0, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 16 }}>
              <p style={{ fontWeight: 700, fontSize: 11, letterSpacing: "0.1em", color: "var(--muted)", margin: "0 0 12px" }}>{t("autoScrollTitle")}</p>
              <button
              ...
              >
                {autoFollow ? t("autoFollowOn") : t("autoFollow")}
              </button>
              <button
              ...
              <p style={{ color: "var(--muted)", fontSize: 12, margin: "0 0 6px" }}>{t("scrollSpeed")} <strong style={{ color: "var(--text)" }}>{scrollSpd}x</strong></p>
              ...
              <p style={{ color: "var(--muted2)", fontSize: 11, marginTop: 10 }}>
                {t("shortcut")} <kbd style={{ background: "var(--surface3)", border: "1px solid var(--border2)", padding: "1px 6px", borderRadius: 4, color: "var(--muted)" }}>A</kbd>
              </p>
            </div>

            {/* Prática: velocidade + pitch + metrônomo */}
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 16 }}>
              <p style={{ fontWeight: 700, fontSize: 11, letterSpacing: "0.1em", color: "var(--muted)", margin: "0 0 12px" }}>{t("practiceTitle")}</p>
              {isPro ? (
                <>
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--muted)", marginBottom: 4 }}>
                      <span>{t("speed")}</span><strong style={{ color: "var(--text)" }}>{speed.toFixed(2)}x</strong>
                    </div>
                    <input type="range" min={0.5} max={1.25} step={0.05} value={speed} onChange={e => setSpeed(Number(e.target.value))} style={{ width: "100%" }} aria-label={t("speed")} />
                  </div>
                  <div>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "var(--muted)", marginBottom: 4 }}>
                      <span>{t("pitchLabel")}</span><strong style={{ color: "var(--text)" }}>{pitch > 0 ? `+${pitch}` : pitch} st</strong>
                    </div>
                ...
                <p style={{ fontSize: 12, color: "var(--muted2)", margin: 0 }}>{t("proSpeedPitch")}</p>
              )}
                ...
                {beats.length > 0 ? (
                  <>
                    <button onClick={() => setMetronome(v => !v)}
                      ...
                      {metronome ? t("metronomeOn") : t("metronome")}
                    </button>
                    <p style={{ fontSize: 11, color: "var(--muted2)", margin: "6px 0 0" }}>{t("metronomeHint")}</p>
                  </>
                ) : (
                  <p style={{ fontSize: 11, color: "var(--muted2)", margin: 0 }}>{t("metronomeNoBeats")}</p>
                )}
              </div>
            </div>
            ...
            <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, margin: "0 0 12px" }}>
                <p style={{ fontWeight: 700, fontSize: 11, letterSpacing: "0.1em", color: "var(--muted)", margin: 0 }}>{t("infoTitle")}</p>
                {isPro && <span style={{ fontSize: 10, color: "var(--muted2)" }}>{t("infoEditHint")}</span>}
              </div>
              {(() => {
                ...
                return (
                  <>
                    <div style={rowStyle}>
                      <span style={labelStyle}>{t("infoKey")}</span>
                      {isPro
                        ? <input value={songKey} onChange={e => setSongKey(e.target.value)} onBlur={() => saveMeta({ key: songKey })} placeholder="?" style={inputStyle} aria-label={t("infoKey")} />
                    ...
                    <div style={rowStyle}>
                      <span style={labelStyle}>{t("infoGenre")}</span>
            ...
            {!isPro && (
              <div style={{ background: "linear-gradient(135deg, #ffffff 0%, #fff4e0 100%)", border: "1px solid rgba(255,154,0,0.25)", borderRadius: 12, padding: 16, textAlign: "center" }}>
                ...
                <p style={{ color: "var(--muted)", fontSize: 12, lineHeight: 1.5, margin: "0 0 12px" }}>{t("upsellText")}</p>
