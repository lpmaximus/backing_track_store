  ...
  if (shapes.length === 0) {
    return (
      <div style={popoverStyle}>
        ...
        <p style={{ color: "var(--muted)", fontSize: 12, margin: "8px 0 0" }}>{t("diagramUnavailable")}</p>
      </div>
    );
  }
      ...
      {shapes.length > 1 && (
        <>
          <div style={{ display: "flex", justifyContent: "center", gap: 5, marginTop: 8 }}>
            {shapes.map((_, i) => (
              <button key={i} onClick={() => setVariation(i)} aria-label={t("variation", { n: i + 1 })}
                style={{
