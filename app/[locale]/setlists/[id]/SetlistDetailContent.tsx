...
export default function SetlistDetailContent({ id }: { id: string }) {
  ...
  async function handleDuplicate() {
    ...
    const name = prompt(t("copyPrompt"), t("copySuffix", { name: setlist?.name ?? t("setlistFallback") }));
    if (name === null) return;
    setDuplicating(true);
    try {
      ...
      const data = await res.json();
      if (res.ok) router.push({ pathname: "/setlists/[id]", params: { id: String(data.setlist.id) } });
      else alert(data.error ?? t("errDuplicate"));
    } finally {
  ...
  async function handleDeleteSetlist() {
    if (!confirm(t("confirmDeleteSetlist", { name: setlist?.name ?? "" }))) return;
  ...
  async function removeItem(itemId: number, title: string) {
    if (!confirm(t("confirmRemoveSong", { title }))) return;
    ...
    <div style={{ flex: 1, maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px", width: "100%" }}>
      ...
      {status === "loading" || loading ? (
        ...
        <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 40, textAlign: "center" }}>
          <p style={{ color: "var(--muted)", fontSize: 14, margin: 0 }}>{t("notFound")}</p>
        </div>
      ) : (
        <>
          {/* Header / metadata */}
          ...
          {songs.length > 0 && (
            <div style={{ marginBottom: 20 }}>
              <Link
                href={{ pathname: "/setlists/[id]/palco", params: { id: String(id) } }}
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: showAddPanel ? 12 : 0 }}>
                <button onClick={() => setShowAddPanel(v => !v)} className="btn-primary" style={{ padding: "9px 20px", fontSize: 13 }}>
                  {showAddPanel ? t("closeSearch") : t("addSongs")}
                </button>
              </div>
              {showAddPanel && (
                <AddSongPicker
                ...
                <div key={s.id} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 18px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <Link
                      ...
                      href={{ pathname: "/song/[slug]", params: { slug: s.slug }, query: { sl: String(s.id) } }}
                    ...
                    >
                      ...
                      <p style={{ color: "var(--muted)", fontSize: 13, margin: 0 }}>{t("songLine", { artist: s.artist, key: s.key, bpm: s.bpm })}</p>
                    </Link>
                    {canEdit && (
                  ...
                  {editingItem === s.id ? (
                    <div style={{ marginTop: 12, paddingLeft: 40 }}>
                      ...
                      />
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => saveItemNote(s.id)} className="btn-primary" style={{ padding: "6px 16px", fontSize: 12 }}>{t("save")}</button>
  ...
  return (
    <div style={{ marginTop: 28, background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 20 }}>
      ...
      {loading ? (
        ...
        <p style={{ color: "var(--muted2)", fontSize: 13, fontStyle: "italic", margin: 0 }}>{t("boardEmpty")}</p>
      ) : (
            ...
            <div key={c.id} style={{ display: "flex", gap: 10 }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                <p style={{ margin: "0 0 2px", fontSize: 13 }}>
                  <strong style={{ color: "var(--text)" }}>{c.userName ?? t("memberFallback")}</strong>
