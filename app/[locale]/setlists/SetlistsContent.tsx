    ...
    <div style={{ flex: 1, maxWidth: 900, margin: "0 auto", padding: "32px 24px 60px", width: "100%" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, gap: 16 }}>
        ...
        {isPro && (
          <button onClick={() => setShowForm(v => !v)} className="btn-primary" style={{ padding: "10px 20px", fontSize: 13, whiteSpace: "nowrap" }}>
            {showForm ? tc("cancel") : t("newSetlist")}
          </button>
        )}
      </div>
      ...
      {accessLoading ? null : !session?.user ? (
        ...
        <div style={{ background: "linear-gradient(135deg, #ffffff 0%, #fff4e0 100%)", border: "1px solid rgba(255,154,0,0.25)", borderRadius: 12, padding: 28, textAlign: "center" }}>
          ...
          <Link href="/planos" className="btn-primary" style={{ padding: "10px 24px", fontSize: 13, display: "inline-block" }}>{t("tryFree")}</Link>
        </div>
      ) : (
        <>
          {showForm && (
            <form onSubmit={handleCreate} style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 12, padding: 20, marginBottom: 20 }}>
              <p style={{ fontWeight: 700, fontSize: 11, letterSpacing: "0.1em", color: "var(--muted)", margin: "0 0 14px" }}>{t("newSetlistCaps")}</p>
              <input
                value={name}
              ...
              {bands.length > 0 && (
                ...
                >
                  ...
                  {bands.map(b => (
                    <option key={b.id} value={b.id}>{t("bandOption", { name: b.name })}</option>
                  ))}
                </select>
              )}
          ...
          {loading ? (
            <p style={{ color: "var(--muted)", fontSize: 13, textAlign: "center", padding: "40px 0" }}>{t("loadingList")}</p>
          ) : items.length === 0 ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {items.map(s => (
                <Link key={s.id} href={{ pathname: "/setlists/[id]", params: { id: String(s.id) } }} style={{
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <p style={{ color: "var(--muted)", fontSize: 13, margin: 0 }}>
                      {t("songCountUpdated", { count: s.songCount, date: formatDate(s.updatedAt) })}
                    </p>
                  </div>
