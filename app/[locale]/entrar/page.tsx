      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", marginBottom: 28 }}>
          ...
          <p style={{ color: "var(--muted)", fontSize: 14, margin: 0 }}>
            {tab === "login" ? t("title") : t("signUpButton")}
          </p>
        </div>
          ...
          {(["login", "cadastro"] as const).map(tabKey => (
            <button key={tabKey} onClick={() => { setTab(tabKey); setError(""); }}
              style={{
                ...
                background: tab === tabKey ? "var(--surface3)" : "transparent",
        ...
        <form onSubmit={handleCredentials} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          {tab === "cadastro" && (
            <input
              type="text" placeholder={t("name")} value={name} onChange={e => setName(e.target.value)}
              required
          ...
          <button type="submit" disabled={loading} className="btn-primary"
            ...
            {loading ? tc("loading") : tab === "login" ? t("signInButton") : t("signUpButton")}
          </button>
        </form>
